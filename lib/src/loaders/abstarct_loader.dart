...
class LottieCache {
  final Map<Object, Future<DotLottie>> _pending = <Object, Future<DotLottie>>{};
  final Map<Object, DotLottie> _cache = <Object, DotLottie>{};
  ...
  Future<DotLottie> putIfAbsent(
    Object key,
    Future<DotLottie> Function() loader,
  ) {
    ...
    if (result != null) {
      ...
      pendingResult.then<void>((DotLottie data) {
        _pending.remove(key);
        _add(key, data);
    ...
    if (result != null) {
      _add(key, result!);
      return SynchronousFuture<DotLottie>(result!);
    }
    assert(_cache.length <= maximumSize);
    return pendingResult!;
  }

  void _add(Object key, DotLottie result) {
    if (maximumSize > 0) {
      assert(_cache.length < maximumSize);
      _cache[key] = result;
    }
